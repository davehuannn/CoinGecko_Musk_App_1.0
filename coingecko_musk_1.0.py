...
crypto_list = [{'id': crypto['id'], 'name': crypto['name']} for crypto in data]
# ------------------------------------------------------------------------------------------------

# CoinGecko 2 (API Extraction)
# ------------------------------------------------------------------------------------------------
def get_crypto_quotes_info(crypto_ids):
    params = {
        'ids': ','.join(crypto_ids),
        'vs_currencies': 'usd'
    }
...
def send_email(subject, body, to_address, attachment_paths):
    ...
    smtp_port = 587
    smtp_username = 'XXXXXXXXXX@gmail.com'  # Replace with your Gmail email address
...
def main():
    ...
    crypto_quotes_info = get_crypto_quotes_info(crypto_ids)

    # Update matching_words_info with additional information
    for entry in matching_words_info:
        crypto_id = entry['crypto_id']
        info = crypto_quotes_info[crypto_id]

        entry['name'] = entry['word']  # CoinGecko doesn't return name in this call, use original name
        entry['symbol'] = info.get('symbol', 'N/A')
        entry['price'] = info['usd']
    ...
    to_email_address = 'XXXXXXXXXX@gmail.com'  # Replace with your email address
